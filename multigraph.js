/*****************************************************************************************************************
                   MiltiGraph creation script - creates bar graphs and scatter graphs
                       Version 1.0.1 - written by Mark Wilton-Jones 03-07/09/2003
******************************************************************************************************************
...
function MWJ_graph(oW,oH,oType,oLegend,oRotate,oColor) {
	...
	this.colours = []; this.labels = []; this.data = []; this.opacities=[];
	...
	this.addDataSet = function (oCol,oLab,oData, oOpacity) {
		...
		this.opacities[this.opacities.length] = oOpacity;
	...
	this.buildGraph = function () {
		...
		if( this.error ) { output+=('<div><font color="#ff0000">Graph drawing failed because '+this.error+'.<\/font><\/div>'); return; }
		var leftTitle = this.rotate ? this.XTitle : this.YTitle;
		var botTitle = this.rotate ? this.YTitle : this.XTitle;
		output+=('<table class="mwjgraph" cellpadding="0" cellspacing="7" border="0">');
		var temp = 1 + ( leftTitle ? 1 : 0 ) + ( this.legend ? 1 : 0 );
		if( this.graphTitle ) { output+=('<tr><th'+((temp>1)?(' colspan="'+temp+'"'):'')+(document.layers?'><font size="+1"':' style="font-size:larger;"')+'>'+this.graphTitle+(document.layers?'<\/font>':'')+'<\/th><\/tr>\n'); }
		output+=('<tr>'); var temp = 1 + ( botTitle ? 1 : 0 );
		if( leftTitle && typeof(leftTitle) == 'string' ) {
			output+=('<th'+((temp>1)?(' rowspan="'+temp+'"'):'')+'>');
		...
		output+=('<table cellpadding="0" cellspacing="0" border="0"><tr><td rowspan="2" valign="top" nowrap>'+(document.layers?('<ilayer height="'+this.height+'">'):('<div style="position:relative;left:0px;top:0px;height:'+this.height+'px;">')));
		//start left scale
		if( this.rotate ) { for( var x = 0; this.XAxis && x < this.XAxis.length; x++ ) { if( this.XAxis[x] ) { output+=(MWJ_giveWords(0,( this.height * ( ( ( 2 * x ) + 1 ) / ( 2 * this.XAxis.length ) ) ) - 7,this.XAxis[x])); } } } else {
			for( var x = 0; x < sideTicks.length; x++ ) { var y = (this.height*((maxH-sideTicks[x])/grRange))-7;
				if( sideTicks[x] == minH ) { y -= 5; } if( sideTicks[x] == maxH ) { y += 4; } output+=(MWJ_giveWords(0,y,sideTicks[x]+((this.type==2)?'%':''))); }
			var y = (this.height*(maxH/grRange))-7;
			if( !minH ) { y -= 5; } if( !maxH ) { y += 4; } output+=(MWJ_giveWords(0,y,0+((this.type==2)?'%':'')));
		}
		...
		output+=((document.layers?'<\/ilayer>':'<\/div>')+'<\/td><td height="'+this.height+'" width="'+this.width+'"'+(isIE5Mac?(' style="position:relative;left:20px;top:0px;" bgcolor="'+(this.bgColor?this.bgColor:'#bfbfbf')+'"><div style="position:absolute;">'):(document.layers?('><ilayer left="0" top="0" width="'+this.width+'" height="'+this.height+'" bgcolor="'+(this.bgColor?this.bgColor:'#bfbfbf')+'">'):('><div style="position:relative;top:0px;left:0px;width:'+this.width+'px;height:'+this.height+'px;background-color:'+(this.bgColor?this.bgColor:'#bfbfbf')+';">'))));
		if( document.layers && !navigator.mimeTypes['*'] ) { output+=('<layer left="0" top="0"><table cellpadding="0" cellspacing="0" border="0"><tr><td height="'+this.height+'" width="'+this.width+'" bgcolor="'+(this.bgColor?this.bgColor:'#bfbfbf')+'">&nbsp;</td></tr></table></layer>'); }
		//start grid
		if( this.type == 3 ) { for( var x = 0; x < botTicks.length; x++ ) { output+=(MWJ_giveBar(10,(this.width*((botTicks[x]-minW)/grWange))-1,(this.height*(maxH/grRange))-5,false)); }
		} else { for( var x = 0; x < botTicks.length; x++ ) { output+=(MWJ_giveBar(grSz,(this.rotate?0:(botTicks[x]-1)),(this.rotate?(botTicks[x]-1):0),this.rotate)); } }
		for( var x = 0; x < sideTicks.length; x++ ) { var y = (this.rotate?(this.width*((sideTicks[x]-minH)/grRange)):(this.height*((maxH-sideTicks[x])/grRange)))-1; output+=(MWJ_giveBar(10,(this.rotate?y:((this.type==3)?(((-1*minW*this.width)/grWange)-5):-5)),(this.rotate?(this.height-5):y),!this.rotate)); }
		var y = (this.rotate?(this.width*((0-minH)/grRange)):(this.height*(maxH/grRange)))-1; output+=(MWJ_giveBar((this.rotate?(this.height+5):(this.width+((this.type==3)?0:5))),(this.rotate?y:((this.type==3)?0:-5)),(this.rotate?0:y),!this.rotate));
		if( this.type == 3 ) { output+=(MWJ_giveBar(10,((-1*minW*this.width)/grWange)-5,(this.height*(maxH/grRange))-1,true));
			output+=(MWJ_giveBar(this.height,((-1*minW*this.width)/grWange)-1,0,false)); }
		//end grid, start bars
		if( !this.type ) {
			for( var x = 0; x < this.data[0].length; x++ ) { for( var y = 0; y < this.data.length; y++ ) { if( this.data[y][x] ) {
				...
				output+=(MWJ_giveBlock(this.rotate?offSet:offSide,this.rotate?offSide:offSet,this.rotate?scaleSize:offWidth,this.rotate?offWidth:scaleSize,this.colours[y],this.data[y][x]+(this.labels[y]?(' - '+this.labels[y]):'')+((this.XAxis&&this.XAxis[x])?(' - '+this.XAxis[x]):''), this.opacities[y]));
			} } }
			...
			for( var x = 0; x < this.data[0].length; x++ ) { for( var y = (this.rotate?0:(this.data.length-1)), totData = 0; ( this.rotate && y < this.data.length ) || ( !this.rotate && y >= 0 ); y += (this.rotate?1:-1) ) { if( this.data[y][x] ) {
				...
				output+=(MWJ_giveBlock(this.rotate?offSet:offSide,this.rotate?offSide:offSet,this.rotate?scaleSize:offWidth,this.rotate?offWidth:scaleSize,this.colours[y],this.data[y][x]+' of '+totCols[x]+((this.type==2)?(' = '+(Math.round((this.data[y][x]/totCols[x])*10000)/100)+'%'):'')+(this.labels[y]?(' - '+this.labels[y]):'')+((this.XAxis&&this.XAxis[x])?(' - '+this.XAxis[x]):''), this.opacities[y]));
			} } }
			...
			output+=(MWJ_giveBlock(xPos-2,yPos-2,4,4,this.colours[y],'['+this.data[y][x][0]+','+this.data[y][x][1]+']'+(this.labels[y]?(' - '+this.labels[y]):'')));
		} } }
		//end bars
		output+=((document.layers?'<\/ilayer>':'<\/div>')+'<\/td><\/tr><tr><td valign="top"'+(isIE5Mac?' height="20"':'')+'>'+(document.layers?('<ilayer width="'+this.width+'" height="20">'):('<div style="position:relative;left:0px;top:0px;height:20px;width:'+this.width+'px;">')));
		//start bottom scale
		if( this.rotate ) {
			for( var x = 0; x < sideTicks.length; x++ ) { var y = (this.width*((sideTicks[x]-minH)/grRange))-((''+sideTicks[x]+((this.type==2)?'  ':'')).length*2);
				if( sideTicks[x] == minH ) { y += ((''+sideTicks[x]+'').length*2); } if( sideTicks[x] == maxH ) { y -= ((''+sideTicks[x]+((this.type==2)?'%':'')).length*4); } output+=(MWJ_giveWords(y,5,sideTicks[x]+((this.type==2)?'%':''),true)); }
			var y = (this.width*(0-minH/grRange))-2;
			if( !minH ) { y += 2; } if( !maxH ) { y -= 4; } output+=(MWJ_giveWords(y,5,0+((this.type==2)?'%':''),true));
		} else if( this.type == 3 ) {
			for( var x = 0; x < botTicks.length; x++ ) { var y = (this.width*((botTicks[x]-minW)/grWange))-((''+botTicks[x]+'').length*2);
				if( botTicks[x] == minW ) { y += ((''+botTicks[x]+'').length*2); } if( botTicks[x] == maxW ) { y -= ((''+botTicks[x]+'').length*4); } output+=(MWJ_giveWords(y,5,botTicks[x],true)); }
		} else { for( var x = 0; this.XAxis && x < this.XAxis.length; x++ ) { if( this.XAxis[x] ) { output+=(MWJ_giveWords( ( this.width * ( ( ( 2 * x ) + 1 ) / ( 2 * this.XAxis.length ) ) ) - ((''+this.XAxis[x]+'').length*3),5,this.XAxis[x],true)); } } }
		//end bottom scale - now end graph area
		output+=((document.layers?'<\/ilayer>':'<\/div>')+'<\/td><\/tr><\/table><\/td>');

		//end of the graph area, resume chart decoration
		if( this.legend ) {
			output+=('<td'+((temp>1)?(' rowspan="'+temp+'"'):'')+'><table cellpadding="0" cellspacing="0" border="1"><tr><td>'+
				'<table cellpadding="0" cellspacing="4" border="0"><tr><th colspan="2">Legend<\/th><\/tr>');
			for( var x = 0, y = ''; x < this.data.length; x++ ) { alert(this.opacities[x]); y += '<tr><td bgcolor="'+this.colours[x]+'" width="20">&nbsp;<\/td><td>'+(this.labels[x]?this.labels[x]:'Untitled')+'<\/td><\/tr>'; }
			output+=(y+'<\/table><\/td><\/tr><\/table>\n');
		}
		if( botTitle ) { output+=('<tr><th>'+botTitle+'<\/th><\/tr>'); }
		output+=('<\/table>\n');
...
function MWJ_giveBlock(oX,oY,oW,oH,oC,oT, oO) {
	...
	} else { return '<div style="position:absolute;left:'+oX+'px;top:'+oY+'px;width:'+oW+'px;height:'+oH+'px;clip:rect(0px,'+oW+'px,'+oH+'px,0px);background-color:'+oC+';cursor:crosshair;" onclick="alert(this.title);" title="'+oT+'" style="opacity:' + oO/100 + ';moz-opacity:' + oO/100 + ';filter:alpha(opacity=' + oO + ');"><\/div>'; }
}
